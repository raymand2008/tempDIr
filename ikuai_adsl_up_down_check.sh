#!/bin/bash
#查询爱快路由器WAN接口是否存在10段的IP，如果存在尝试启停adsl线路，并推送消息到微信
...
echo ""
echo "[`date "+%Y-%m-%d %H:%M:%S"`]adsln NO:------>[wanids]["$wanids"]bug"
...
if [ $needrebootip -eq 0 -o "a$needrebootip" == "a0" ];then
   echo "[`date "+%Y-%m-%d %H:%M:%S"`]needrebootip的值为：[$needrebootip]  无异常线路 !"
fi

if [ $needrebootip -eq 1 -o "a$needrebootip" == "a1" ];then
   
   curl -sSfL 'http://192.168.6.1/Action/call' \
   -H 'Connection: keep-alive' \
   -H 'Pragma: no-cache' \
   -H 'Cache-Control: no-cache' \
   -H 'Accept: application/json, text/plain, */*' \
   ...
   -H 'Referer: http://192.168.6.1/' \
   -H 'Accept-Language: zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7,de;q=0.6,zh-TW;q=0.5' \
   --data-binary '{"func_name":"wan","action":"vlan_down","param":{"id":"'`echo "${wanids}"|tr "\n" ","|sed -e 's/,$//'`'"}}' \
   -b $cookieFile
   ...
   echo "[`date "+%Y-%m-%d %H:%M:%S"`][had stop wait start]"[--data-binary '{"func_name":"wan","action":"vlan_down","param":{"id":"'`echo "${wanids}"|tr "\n" ","|sed -e 's/,$//'`'"}}']debug!
   ...
   -H 'Referer: http://192.168.6.1/' \
   -H 'Accept-Language: zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7,de;q=0.6,zh-TW;q=0.5' \
   --data-binary '{"func_name":"wan","action":"vlan_up","param":{"id":"'`echo "${wanids}"|tr "\n" ","|sed -e 's/,$//'`'"}}' \
   -b $cookieFile
   ...
   echo "[`date "+%Y-%m-%d %H:%M:%S"`][had start]"[--data-binary '{"func_name":"wan","action":"vlan_up","param":{"id":"'`echo "${wanids}"|tr "\n" ","|sed -e 's/,$//'`'"}}']debug!
...
#   推送pushplus 消息，pushplus微信关注 订阅激活消息， 替换token值即可
   ...
   -H 'Accept-Language: zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7,de;q=0.6,zh-TW;q=0.5' \
   --data-binary '{"token":"c207657d4e064ce8bc97dc25ce0ab3df","title":"翡翠园公网adsl线路告警","content":"'`echo "$(date +%Y-%m-%d%H:%M:%S)"`'翡翠园公网线路告警存在10线路，已尝试修复","template":"html","channel":"wechat","webhook":""}' \
   -b $cookieFile
   ...
   if [ "a" != "a$wanids" ];then
     echo "" > rebootnet$1.data
     echo "[`date "+%Y-%m-%d %H:%M:%S"`][reboot]over ok!"
   else
     echo "[`date "+%Y-%m-%d %H:%M:%S"`][reboot]over err!"
   fi
 else
    echo "[`date "+%Y-%m-%d %H:%M:%S"`]needrebootip的值为：[$needrebootip]  !"
fi
